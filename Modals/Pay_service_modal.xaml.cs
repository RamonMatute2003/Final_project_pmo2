...
namespace Final_project.Modals{
	public partial class Pay_service_modal : ContentPage{
        ...
        double amount_user, amount_user_send;
        ...
        private async Task<bool> insert_record(){
            ...
            Table_records record = new Table_records(Temporary_data.id_sender_user, 1, Temporary_data.amount, generate_code(), generate_date(),description, Temporary_data.id_service);
            string response = "";
        ...
        private string generate_code() {
            ...
            return date.Second.ToString()+random.Next(1000,1000).ToString()+date.Minute.ToString()+random.Next(10000,10000).ToString();
        }
